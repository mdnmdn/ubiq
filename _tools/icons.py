...
# The generated enum, and the asset paths behind it. Written rather than hand-kept, because a
# registry row and a Rust variant that drift apart is a blank square nobody notices.
GENERATED = ROOT / "crates" / "ubiq" / "src" / "ui" / "kit" / "icons.rs"
# `include_bytes!` resolves against the generated file's own directory.
UP = "../" * 5


def camel(name: str) -> str:
    """`tab-pin` -> `TabPin`: the registry key as a Rust variant."""
    return "".join(part.capitalize() for part in name.split("-"))


def generated(icons: dict[str, Icon]) -> str:
    """The whole of `crates/ubiq/src/ui/kit/icons.rs`, from the rows that have a file of ours."""
    ours = sorted(
        (i for i in icons.values() if not i.borrowed and i.path.exists()),
        key=lambda i: i.name,
    )
    variants = "\n".join(f"    /// {i.goal}\n    {camel(i.name)}," for i in ours)
    paths = "\n".join(f'            Self::{camel(i.name)} => "icons/{i.name}.svg",' for i in ours)
    all_of = "\n".join(f"    UbiqIcon::{camel(i.name)}," for i in ours)
    bytes_arms = "\n".join(f'        "icons/{i.name}.svg" => ours!("{i.name}"),' for i in ours)
    return f'''//! Ubiq's own icons, as Rust — **generated by `just icons-gen`, do not edit**.
//!
...
macro_rules! ours {{
    ($name:literal) => {{
        include_bytes!(concat!("{UP}assets/icons/", $name, ".svg")).as_slice()
    }};
}}
...
def main() -> int:
    ...
    sub.add_parser("codegen", help="write crates/ubiq/src/ui/kit/icons.rs from the registry")
    ...
    if args.cmd == "codegen":
        GENERATED.write_text(generated(icons))
        console.print(f"[green]{GENERATED.relative_to(ROOT)}[/]")
        return 0

    if args.cmd == "check":
        problems, pending = check(icons)
        # A row added without regenerating is a variant that does not exist yet, so the drift is
        # a mechanical problem like any other rather than something to notice at the next build.
        current = GENERATED.read_text() if GENERATED.exists() else ""
        if current != generated(icons):
            problems.append("[red]icons.rs is out of date — run `just icons-gen`[/]")
