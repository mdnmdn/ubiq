...
└── src/                   # implementation (core + all phases)
    ...
    ├── isolate.rs         # RunSpec -> isol8 Spec/Context in-process; confined_launch execs sandbox-exec on macOS, writes an am-confine shim launch on Windows, errors on Linux; login capture confines inherited-stdio runs incl. Windows (core, P3)
    ...
    └── bin/
        └── am-confine.rs  # Windows pane shim: reads a ConfinePayload, lets isol8 spawn the
                            # harness from inside the caller's own console (no feature gate —
                            # a lib-mode embedder needs it beside its own binary)
```
...
**Phase 3 ✅**
- [x] isolation (`--isolate[=profile]` / `--no-isolate` via isol8, a core library dependency, not an external binary): `src/isolate.rs` builds the policy in-process; settings `[isolate] enabled|profile|home`; confining a run in a caller-owned terminal works on macOS (execs `sandbox-exec`) and on Windows (writes an `am-confine` shim launch, since isol8 has no ConPTY seam but spawns with no console-creation flag, so the shim's child lands on the caller's own console) — Linux still errors, see `refs/isol8-pty-seam-update.md`; inherited-stdio runs (`am account login --isolate`) confine on Windows too (hook DLL, isol8 v0.4.0)
