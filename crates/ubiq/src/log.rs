...
pub const DEFAULT_FILTER: &str = "ubiq=debug,agent_manager=debug,gpui_terminal=debug,warn";
...
impl Subsystem {
    ...
    /// Which subsystem a `tracing` target belongs to. The more specific prefixes are tested first,
    /// because `ubiq::pty` is also `ubiq`.
    fn of(target: &str) -> Subsystem {
        if target.starts_with("ubiq::pty") {
            Subsystem::Pty
        } else if target.starts_with("ubiq::orchestrator") || target.starts_with("ubiq::bus") {
            Subsystem::Coordinator
        } else if target.starts_with("ubiq::mcp_server") {
            Subsystem::Mcp
