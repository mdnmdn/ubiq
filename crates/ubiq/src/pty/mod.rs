...
impl Pty {
    ...
    pub fn forward_output(&self, pane_id: Uuid, to_ui: flume::Sender<Message>) -> Result<()> {
        let mut reader = self
            .master
            .try_clone_reader()
        ...
        thread::spawn(move || {
            ...
            loop {
                match reader.read(&mut buffer) {
                    ...
                    Ok(n) => {
                        let sent = to_ui.send(Message::TerminalOutput {
                            pane_id,
...
pub fn reap(
    ...
    thread::spawn(move || {
        let code = child
            ...
            .unwrap_or(-1);
        let _ = to_ui.send(Message::PaneExited { pane_id, code });
    });
}
