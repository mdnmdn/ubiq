//! The one fixture left.
//!
//! Projects, the file tree, a file's bytes and the panes all come from the host now, and the
//! constructors that invented them are gone. The chat is the last thing here because the chat has
//! no transport family: its composer sends to nothing and its reply is canned, so a fixture is
//! still the honest way to draw it. It goes the same way when it gets one.
