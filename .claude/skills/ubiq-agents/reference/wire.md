...
(`sandbox-exec`) and on **Windows** (the running binary re-invoked in the pane under
`isolate::CONFINE_ARG`, since isol8 has no ConPTY seam but its Windows backend attaches an
unflagged child to whatever console its caller runs in — `confine_entrypoint()` is the embedder's
hook, and no second executable ships); Linux still errors, and the seam that would close it is
