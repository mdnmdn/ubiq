...
| [Decision register](./tech/decisions.md) | One entry per structural decision — what was chosen, why, and what it costs — cited as `Dnn` across this library. | 2026-09-14 |
...
| [Claude credentials vanish from a run directory](./wip/claude-auth-problem.md) | A Claude Code pane loses its OAuth login after some hours. The machine evidence, the credential path from the macOS Keychain through the seed into a run directory and back, the hypotheses and what would falsify each, the instrumentation added to settle it, and the standing watch log. | 2026-09-14 |
