...
| Document | What it is | Verified |
|---|---|---|
| [Backlog](./backlog.md) | Every open question, known gap and deferred item across the project, in one register. | 2026-09-19 |
...
| [In-place help, and the identity layer under it](./wip/in-place-help.md) | An inspector-style help mode — point at any part of the window and read what it is — and the element identity scheme underneath it, which exists to serve personalisation and extensions as much as help. Phase 1 is built — the `UiId` grammar, a static catalogue of names, a `.ui_id()` element extension feeding a per-frame bounds registry, and the deepest-hit lookup the overlay will call. Phases 2 and 3 — the targeting mode, the overlay, and the first identified screens — are designed here and not written. | 2026-09-20 |
