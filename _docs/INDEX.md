...
| [Project structure](./tech/project-structure.md) | Every folder in the workspace, what belongs in it, what must never go in it, and the two crates' division of labour. | 2026-09-12 |
...
| [The drone, phase by phase](./wip/drone.md) | The running state of the drone's nine phases — one small executable Ubiq places on another machine to serve its terminal, files and machine facts over a single duplex byte stream. Phases 1 and 2 are built and verified; the handshake, the SSH connection store, the interface, the lifecycle and the deployment are not. This document is the ledger, updated as each phase lands. | 2026-09-12 |
