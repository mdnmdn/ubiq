...
| The MCP surface Ubiq injects | `crates/ubiq-host/src/mcp/` | Tools Ubiq exposes to the agents it hosts, over one loopback listener (`D102`). The catalogue is `test`, `project-info`, `manage-ubiq-tasks`, `use-task` and `ubiq-kb`; the task pair shares `Work` through a handle and posts task mutations to every window (`D120`), and `ubiq-kb` shares `Kb` the same way, mutating through `kb/ops.rs` and posting `KbChanged` |
