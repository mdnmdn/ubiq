...
| A project's knowledge-base roots, a folder or a fetched repository | `crates/ubiq-host/src/kb/` | The root list, one TOML file per project, not the catalogue's; a git root fetches on a thread of its own, on `repos/clone.rs`'s shape, behind `git`. Listing and reading still ride the files worker, filtered by `KbRoot::admits` after the walk |
...
| The MCP surface Ubiq injects | `crates/ubiq-host/src/mcp/` | Tools Ubiq exposes to the agents it hosts, over one loopback listener (`D102`). The catalogue is `test`, `project-info`, `manage-ubiq-tasks`, `use-task`, `ubiq-kb` and `ubiq-plan`; the task pair shares `Work` through a handle and posts task mutations to every window (`D120`), `ubiq-kb` shares `Kb` the same way, mutating through `kb/ops.rs` and posting `KbChanged`, and `ubiq-plan` (`crates/ubiq-host/src/mcp/plan.rs`) shares `crate::plan::Plans` the same way again, over `read_plan`, `write_plan`, `list_annotations`, `reply_annotation` and `resolve_annotation` — a server of its own rather than two more tools on `manage-ubiq-tasks`, so a task-management agent does not carry plan tools it never calls; no tool opens an annotation, only a window does |
...
see `Disconnected` to break its loop on. `crates/ubiq-host/src/mcp/` is the caller: the single
loopback listener `D102` chooses puts a tool call on its own thread with no window on either end
of it. Raising a notification from there means saying `Message::RaiseNotification` exactly as a
window would. Task tools take a shared `work::Handle` rather than asking the coordinator, and post
`TaskCreated` / `TaskChanged` / `TaskDeleted` to every window through a `Mailbox` (`D120`).
