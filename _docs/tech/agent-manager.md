...
the whole machine, with no per-pane override (`G295`). A third, cosmetic fault the same probe's
...
this side zeroes them — inert, never functional (`G296`). All three are written up with their
