...
itself to a strip when it holds no task — and the list is sparse: a lane the project has never been
...
and `WebBundleReady` says which one it got. A bundle is versioned by file, so a host that pins
...
`<shared workarea>/web/<app>/<version>.bundle` — one compressed file, not a directory — and the
interface uses the string it was handed.
