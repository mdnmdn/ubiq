...
updated: 2026-08-31
verified: 2026-08-31
code_anchors: [crates/ubiq/src/messages.rs]
...
against it and every future topology preserves it. `crates/ubiq/src/messages.rs` is its home; the
...
Three records travel inside session-family payloads.
...
1. Decide the family. If it names a pane it belongs to the pane family; otherwise the session
   family.
2. Add the variant to the enum in `crates/ubiq/src/messages.rs`, with an owned payload — no
