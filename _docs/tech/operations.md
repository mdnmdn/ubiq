...
| `just web-assets-drawio [args]` | Re-snapshot the draw.io mirror and rewrite its own hash manifest |
| `just web-assets-verify-drawio` | The drift check, against the draw.io manifest |

Both snapshots are taken when their tenant's pinned version changes, never at runtime, and downloads
land in a scratch directory outside the tree. Excalidraw's manifest is generated by walking its
`dist/prod` off jsDelivr; draw.io's has no npm package to walk — its webapp is mirrored from the
`jgraph/drawio` GitHub repository's tree at a tag, because jsDelivr refuses to list a package that
large, and its own hashes are taken at snapshot rather than read from the source, since GitHub
publishes none either. `_tools/webassets.py snapshot --tenant {excalidraw,drawio}` is the one script
behind every one of these four recipes; `verify` reads whichever manifest `--out` names and needs no
