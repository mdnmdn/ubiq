...
| **Claude**, **Codex** and **Grok** accept a second turn on the same process (`IoSupport::multi_turn`). opencode and Copilot bridges are one-shot: the prompt goes in through argv, the process answers once and exits, and `send` on that process is a no-op | All five still back a conversation column. A one-shot harness's turn ending is a process exiting, not the conversation — the coordinator's `finish_one_shot_turn` relaunches it with the harness's own session id on the next prompt (`RunSpec::resume`), so the composer never has to know which kind it is talking to |
| **Four of five bridges auto-approve.** Claude's reader answers every `control_request` with `allow`; Codex auto-accepts every approval RPC; opencode runs `--dangerously-skip-permissions`; Copilot runs `--allow-all --no-ask-user`. `AcpBridge` is the exception: it parks `session/request_permission` by request id and waits for an `AgentInput::AnswerPermission` | A permission prompt in the UI is theatre for the four — the tool has already run. This is the one item with a security consequence, and it gates any "ask me first" feature |
...
| `to_acp` is a **stateless one-event mapper** that drops `ApprovalRequest`, `Usage`, `Result` and `SessionStarted`, and emits no JSON-RPC envelope, no session id and no turn brackets | There is no ACP *server* today. `io/acp_client.rs` is the client half, with the envelope and the session table it owns; the outbound projection stays a projection |
...
two variations to diff: one conversation, two wire formats, one rendered result. The client bridge
it needs is `io/acp_client.rs`, and it is generic, so the work is a `harness_identity!` entry and an
argv rather than a bridge.
