...
`AgentInput::SetConfigOption` outright — `io/jsonl.rs:222`, `io/codex.rs:439`. So a model picker has
to be answered *before* the harness starts, which is what shapes P3 below. An ACP harness is the
exception and the reason P3 is not the whole answer: `AcpBridge` answers `SetConfigOption` with a
`session/set_config_option` (`io/acp_client.rs:881`), and every adapter captured so far advertises a
`model` and a `mode` category on `session/new` — so for those a picker could be live rather than
pre-launch. Nothing renders one yet.
