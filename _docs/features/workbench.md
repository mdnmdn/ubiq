...
the dialled host to the window's `Bus` as a `HostRef::Remote`, but stops there: nothing in this
build lists the connections a window is holding, offers a saved-hosts list or a host dropdown, or
lets a project be opened on one — [`../backlog.md`](../backlog.md) (`G166`) is the last of those
that G165 does not already cover. The transport underneath the modal —
