...
irreversible one of the eleven.
...
**Two rows after Fork open a second face onto the agent's environment rather than acting on the
agent.** Open terminal (`SpawnWorkspace` with `beside` naming this agent) puts a shell pane in the
folder the harness is working in and under the environment it was launched with — its variables,
...
store mid-append tears the last record), Open terminal and New agent here only while launched,
Delete always — so the menu's shape never changes under
the cursor. The eleven labels and their enablement are one list of pairs,
