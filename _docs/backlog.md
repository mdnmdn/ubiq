...
| G9 | The explorer, the editor and the branch readout render fixtures from `state/sample.rs`. Projects are real; no folder is read, no file is opened, no branch is queried | [`features/workbench.md`](./features/workbench.md) |
...
| G15 | A second window gets its own fixtures rather than its project's real tree, so two windows differ only by name, letter, colour and remembered layout | [`features/workbench.md`](./features/workbench.md) |
