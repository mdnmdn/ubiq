...
| G69 | Project search is built except for the gesture that fires it: contract, host worker, panel and every reply are in the tree, and nothing sends `SearchProject` — no start mutator, no submit on the query field, no option toggles, no row-as-destination, no cancel, no worker tests. §0 of the proposal is the list | [`inbox/omni-search-proposal.md`](./inbox/omni-search-proposal.md) |
