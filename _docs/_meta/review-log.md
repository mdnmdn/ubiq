...
| 2026-09-16 | The drone, on request — executing `P18` and `P19` | L1, L2, L4, L5, L7, L9 | `wip/drone.md` was a 607-line ledger of ten phases, nine of them built and over the 500-line ceiling, holding facts five `tech/` documents own; `tech/transport-contract.md` named a symbol the tree spells `DroneIdentity::has` | Wrote `features/drone.md` (329 lines, `id: feat-drone`) over the built capability, linking the owning documents rather than restating them; cut `wip/drone.md` to phase 10 and the open gaps (190 lines) with its id unchanged; repointed the footers of `tech/architecture.md`, `tech/operations.md` and `tech/project-structure.md` and the backlog rows `G263` and `G264`; added the drone's two rows to `INDEX.md`'s ownership registry and one reading path; corrected the symbol; closed `P18` and `P19` | `G256`–`G258`, `G261`, `G262`, `G265`–`G268` stay pointed at `wip/drone.md` — each is an untested surface or phase-10 work. The four pre-existing lint failures are untouched: `inbox/feedback-tools.md`, `inbox/transport-contract-audit.md`, `wip/claude-auth-problem.md` and `features/workbench.md` |

| 2026-09-24 | `features/workbench.md`, on request — card T-79 | L1, L2, L4, L5, L7 | The document was 5011 lines, ten times the ceiling, and flat: one `## Behaviour` of 3300 lines with no subheading in it, covering nine rail modes and the chrome round them at once | Split it per rail mode, which is what the card authorised: `workbench-ide.md`, `workbench-git.md`, `workbench-agents.md`, `workbench-teams.md`, `workbench-tasks.md` and `workbench-sink.md`, each carrying that mode's Behaviour, Contract, Implementation and Failure rows verbatim and its own `code_anchors`. `workbench.md` keeps the rail, the dock, the panels, the titlebar, the projects and windows model, the settings dialogs, the navigator, the file picker, the status bar, the window-areas tables and a summary of each mode. 69 `backlog.md` rows, six skill and tech references and `INDEX.md` §3 and §4 repointed | The chrome document is still 1951 lines and three of the six new ones are over the ceiling — the card authorised a split per mode and no more, so the second cut is `P24` |
