...
the question [`detached-panes-proposal`](../detached-panes-proposal.md) scoped out for the window
lands: a reattachment that crosses clients. The honest consequence is that the first release is a
terminal interface *to a host*, not a terminal view *of the window* — which is what the SSH case
actually wants.
...
- **`D164` — a second interface is a client, not a mode.** The terminal interface is its own crate
...
- **`G340`** — every streamed resource routes to one client and the host keeps no history of either
...
- **`G341`** — nothing enforces one host per config root. `--own` against a served root is fenced by
...
- **`G342`** — the conversation fold lives in the window's crate; sharing it with a second interface
  is a crate move that has not been made.
- **`G343`** — the dialled transport is token-only and plaintext by default, and the host record puts
...
- **`G344`** — "one application per config root" and "one host per config root" are two mechanisms
