...
- **`D166` — a streamed resource has many viewers and one writer.** A pane's keyboard and a
  ...
  every joiner.
- **`G345`** — a pane's geometry belongs to its lease holder, so a viewer on a smaller terminal
  letterboxes and nothing tells it why.
- **`G346`** — the ring gives a joiner bytes and not a screen: a shell mid-scroll repaints for nobody,
  and only a full-screen harness answers the nudge.
- **`G347`** — whether a live conversation buffer may exist is unsettled between this and
  `inbox-chat-resume`, and building one without settling it makes two designs for one fact.
- **`G348`** — every attached client is the same person by assumption; there is one token, no identity
  and no audit, so the lease is a safety catch and cannot be a permission.
- **`G349`** — reaping is written for one interface: `client_gone` takes panes and conversations with
